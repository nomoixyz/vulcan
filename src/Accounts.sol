...
library AccountsLib {
    ...
    function readStorage(Accounts, address who, bytes32 slot) internal view returns(bytes32) {
    ...
    function sign(Accounts, uint256 privKey, bytes32 digest) internal pure returns (uint8, bytes32, bytes32) {
    ...
    function derive(Accounts, uint256 privKey) internal pure returns (address) {
    ...
    function getNonce(Accounts, address who) internal view returns (uint64) {
        return getNonce(who);
    }
    ...
    function recordStorage(Accounts) internal {
        vulcan.hevm.record();
    }
    ...
    function getStorageAccesses(Accounts, address who) internal returns (bytes32[] memory reads, bytes32[] memory writes) {
        return getStorageAccesses(who);
    }
    ...
    function label(Accounts, address who, string memory lbl) internal returns (address) {
    ...
    function create(Accounts self, string memory name) internal returns (address) {
        return create(self, name, name);
    }

    /// @dev creates an address from `name` and labels it with `lbl`
    function create(Accounts self, string memory name, string memory lbl) internal returns (address) {
        address addr = derive(self, uint256(keccak256(abi.encodePacked(name))));
    ...
    function setStorage(Accounts, address self, bytes32 slot, bytes32 value) internal returns(address) {
        return setStorage(self, slot, value);
        return self;
    }
    ...
    function setNonce(Accounts, address addr, uint64 n) internal returns(address) {
    ...
    function impersonateOnce(Accounts, address sender) internal returns(address) {
    ...
    function impersonate(Accounts, address sender) internal returns(address) {
    ...
    /// @return the `msg.sender` for the next call
    function impersonateOnce(Accounts, address sender, address origin) internal returns(address) {
    ...
    /// @param origin the address to set the `tx.origin`
    /// @return the `msg.sender` for the next calls
    function impersonate(Accounts, address sender, address origin) internal returns(address) {
    ...
    function stopImpersonate(Accounts) internal {
    ...
    /// @return the address that was updated
    function setBalance(Accounts, address self, uint256 bal) internal returns(address) {
    ...
    function setCode(Accounts, address self, bytes memory code) internal returns(address) {
...
Accounts constant accounts = Accounts.wrap(0);

using AccountsLib for Accounts global;